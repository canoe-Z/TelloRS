...
def normalize(K, pts):
    ...
    # turn [[x,y]] -> [[x,y,1]]
    def add_ones(x): return np.concatenate(
...
def extract_points(frame):
    ...
    pts = cv2.goodFeaturesToTrack(
        image, 3000, qualityLevel=0.01, minDistance=3)
    # extract features
...
def process_frame(frame):
    ...
    if frame.idx == 1:
        ...
        points4d = triangulate(
            frame.last_kps, frame.curr_kps, frame.last_pose, frame.curr_pose)
